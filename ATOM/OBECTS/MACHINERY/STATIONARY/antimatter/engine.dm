...
/obj/machinery/power/am_engine/injector/New()
	sleep(5)
	for(var/obj/machinery/power/am_engine/engine/S in locate(x, y+1, z))
		src.connected = S
	..()
	return
...
/obj/machinery/power/am_engine/injector/attackby(obj/item/weapon/fuel/F, mob/user)
	...
	if(istype(F, /obj/item/weapon/fuel/H))
		if(injecting)
			usr << "Theres already a fuel rod in the injector!"
			return
		usr << "You insert the rod into the injector"
		injecting = 1
		var/fuel = F.fuel
		del(F)
		spawn( 300 )
...
/obj/machinery/power/am_engine/engine/New()
	..()
	sleep(2)
	for(var/obj/machinery/power/am_engine/injector/S in locate(x, y-1, z))
		src.connected = S
	..()
	return
