...
/obj/machinery/disposal/toilet
	...
	MouseDrop_T(mob/target, mob/user)
		...
		for (var/mob/V in viewers(usr))
			if(target == user && !user.stat)
				V << "[user] sits on the toilet."
			if(target != user)
				V << "[user] places [target.name] on the toilet."
		target.loc = loc
		return
	...
	interact(mob/user)
		...
		for (var/mob/V in viewers(user))
			V << "[user] eagerly drinks the toilet water!" //Yum yum yum
		return
