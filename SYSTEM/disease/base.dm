/mob/var/list/datum/microorganism/disease/immunemicroorganism = list()
/mob/var/list/datum/microorganism/resistance/resistances2 = list()
/mob/var/list/resistances = list()
/mob/var/radiation = 0.0
/mob/var/mutations = 0

/mob/proc/shake_camera(var/volume)
	var/i = 0
	var/g = rand(2, 4)
	while(i < g)
		spawn(20)
			client.eye = locate(x + rand(-1 * volume, volume), y + rand(-1 * volume, volume), z)
		i += 1
...
/datum/microorganism/effect/alien
	...
	activate(var/mob/simulated/living/mob,var/multiplier)
		...
		if(prob(40))
			if(mob.client)
				mob.client.mob = new/mob/alien/larva(mob.loc)
			else
				new/mob/alien/larva(mob.loc)
