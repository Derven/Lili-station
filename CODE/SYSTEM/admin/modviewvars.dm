/client/proc/cmd_modify_object_variables(obj/O as obj|mob|turf|area in range(1, usr))
	set category = "Debug"
	set name = "Edit Variables"
	set desc="(target) Edit a target item's variables"
	src.modify_variables(O)

/client/proc/cmd_viewvariables(obj/O as obj|mob|turf|area in range(1, usr))
	set category = "Debug"
	set name = "View Variables"
	set desc="(target) Edit a target item's variables"
	O.show_variables(usr, O)

/atom/proc/show_variables(var/mob/M, var/atom/A)
	var/list/varsss = list()
	for(var/v_a_r in A.vars)
		varsss += "[v_a_r] - [A.vars[v_a_r]]"
	special_browse(M, nterface(varsss, varsss))
