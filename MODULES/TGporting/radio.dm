...
/obj/item/device/radio/talk_into(mob/M as mob, message, channel)
	...
	if(GLOBAL_RADIO_TYPE == 1) // NEW RADIO SYSTEMS: By Doohl
	   ...
	   //#### Grab the connection datum ####//
		...
		var/jobname = "radio" // the mob's "job"
	  ...
	  /* ###### Intercoms and station-bounced radios ###### */
		...
		Broadcast_Message(connection, M, voicemask, "*",
						  src, message, displayname, jobname, real_name, "noname",
		                  filter_type)
		...
		if (length(heard_masked) || length(heard_normal) || length(heard_voice) || length(heard_garbled))
			...
			if (length(heard_voice))
				var/rendered = "[part_a]\[[M.gender]\][part_b]["*"][part_c]"

				for (var/mob/R in heard_voice)
					R.show_message(rendered, 2)

			if (length(heard_garbled))
				quotedmsg = M.Say(message)
				var/rendered = "[part_a]\[[M.gender]\][part_b][quotedmsg][part_c]"
...
/obj/item/device/radio/proc/hear_talk(mob/M as mob, msg)
	if (broadcasting)
		talk_into(M, msg)
/*
